...
function renderList(profiles) {
    ...
    profiles.forEach((p, index) => {
        ...
        btnCopy.title = "Copiar Perfil (JSON)";
        btnCopy.onclick = () => {
            const json = JSON.stringify(p, null, 2);
            navigator.clipboard.writeText(json);
            updateStatus('Perfil copiado como JSON!', 'green');
        };
...
function openEditor(profile = null, index = -1) {
    ...
    if (profile) {
        ...
        document.getElementById('projectSelect').value = profile.project || 'agent';
        ...
        document.getElementById('passwordInput').value = profile.password || '';
    } else {
...
async function importProfile() {
    const json = prompt('Cole o JSON do perfil aqui:');
    if (!json) return;
    ...
    try {
        ...
        // Validação básica
        if (!profileData.name) {
            throw new Error('JSON inválido: Campo "name" é obrigatório.');
        }
        ...
        loadProfiles();
        updateStatus('Perfil importado com sucesso!', 'green');
    } catch (err) {
        updateStatus('Erro ao importar: ' + err.message, 'red');
    }
}
